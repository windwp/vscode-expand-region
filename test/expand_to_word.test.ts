...
suite("Tests expand_to_word", () => {
    ...
    test(" test_special_chars2", () => {
        ...
        assert.equal(result.startIndex, 57)
        assert.equal(result.endIndex, 64)
    });
    test(" test_special_chars3", () => {
        let result = expand_to_word(fileData3, 75, 77)
        assert.equal(result.startIndex, 75)
        assert.equal(result.endIndex, 85)
    });
    test(" test_special_chars4", () => {
        let result = expand_to_word(fileData3, 89, 89)
        assert.equal(result.startIndex, 86)
        assert.equal(result.endIndex, 89)
    });
